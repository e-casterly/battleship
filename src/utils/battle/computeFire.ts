...
function buildShotsPatch(
  shots: Shots,
  defenderId: PlayerId,
  updates: Record<string, ShotResult>,
): Shots {
  return {
    ...shots,
    [defenderId]: { ...shots[defenderId], ...updates },
  };
}
...
export function computeFire(
  ...
  if (shipId === undefined) {
    return {
      patch: {
        shots: buildShotsPatch(state.shots, defenderId, { [cellId]: "miss" }),
      },
      result: "miss",
  ...
  if (currentHealth === undefined)
    throw new Error(
      `Ship "${shipId}" not found in fleetHealth for defender "${defenderId}"`,
    );
  const updatedHealth = currentHealth - 1;
  const isSunk = updatedHealth === 0;

  const fleetHealth: FleetHealth = {
    ...state.fleetHealth,
    [defenderId]: { ...state.fleetHealth[defenderId], [shipId]: updatedHealth },
  };

  if (!isSunk) {
    return {
      patch: {
        shots: buildShotsPatch(state.shots, defenderId, { [cellId]: "hit" }),
        fleetHealth,
      },
      result: "hit",
      excludedCells: [cellId],
  ...
  return {
    patch: {
      shots: buildShotsPatch(state.shots, defenderId, {
